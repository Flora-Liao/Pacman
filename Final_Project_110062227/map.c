...
// You can just copy the map data in "	.txt" to the array `default_map`
...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		if(pFile) game_log("fopen succeeded\n");
		if (!pFile) {
...
static void draw_block_index(Map* M, const int row, const int col) {
	...
	dw = block_width / 3;
		...
		al_draw_filled_rectangle(s_x, s_y,
			e_x, e_y, al_map_rgb(255, 255, 255));//lwscolor
		if (row < M->row_num - 1 && B && !(BL && BR && R && L)) {
			...
			al_draw_filled_rectangle(s_x, s_y,
				e_x, e_y, al_map_rgb(255, 255, 255));
		}

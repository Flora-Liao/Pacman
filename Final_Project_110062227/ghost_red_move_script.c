...
#include "map.h"
/* Shared variables */
#define GO_OUT_TIME1 256
#define GO_OUT_TIME2 512
	...
	if (ghost_movable(ghost, M, shortestDirection, true)) {
		...
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, true)) {
				if (ghost->objData.preMove == 1 && i == 4)continue;
				if (ghost->objData.preMove == 4 && i == 1)continue;
				if (ghost->objData.preMove == 2 && i == 3)continue;
				if (ghost->objData.preMove == 3 && i == 2)continue;
				proba[cnt] = i;
				cnt++;
			}
		...
		int flag = 0;
		if (cnt <= 2 && ghost_movable(ghost, M, ghost->objData.preMove, true)) {
...
void ghost_red_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			if (al_get_timer_count(game_tick_timer) > GO_OUT_TIME1)
				ghost->status = GO_OUT;
